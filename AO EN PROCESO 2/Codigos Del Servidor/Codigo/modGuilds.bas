...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).Stats.ELV < 50 Or TieneObjetos(560, 1, UserIndex) = False Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        refError = "Para fundar un clan debes ser nivel 50 , entregar 1 Amuleto del lider y  tener 100 skills en liderazgo."
        Exit Function
    End If
    ...
    Call QuitarObjetos(560, 1, UserIndex)
...
Public Function m_ListaDeMiembrosOnlinePOS(ByVal UserIndex As Integer, ByVal GuildIndex As Integer) As String

Dim i As Integer
    If GuildIndex > 0 And GuildIndex <= CANTIDADDECLANES Then
        i = Guilds(GuildIndex).m_Iterador_ProximoUserIndex
        While i > 0
                m_ListaDeMiembrosOnlinePOS = m_ListaDeMiembrosOnlinePOS & UserList(i).name & ", Posicion: " & UserList(i).Pos.Map & "-" & UserList(i).Pos.x & "-" & UserList(i).Pos.y & ". "
            i = Guilds(GuildIndex).m_Iterador_ProximoUserIndex
        Wend
    End If
    If Len(m_ListaDeMiembrosOnlinePOS) > 0 Then
        m_ListaDeMiembrosOnlinePOS = Left$(m_ListaDeMiembrosOnlinePOS, Len(m_ListaDeMiembrosOnlinePOS) - 1)
    End If
