...
Procesado = True 'ver al final del sub
    ...
    Select Case UCase$(rData)
        Case "/INFORMARSUBASTA"
           If Hay_Subasta = False Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No hay subastas realizandose actualmente." & FONTTYPE_INFO)
                Exit Sub
            End If
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Subasta iniciada por: " & UserList(Subastador).name & FONTTYPE_INFO)
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Objeto Subastandose: " & ObjData(objetosubastado.ObjIndex).name & FONTTYPE_INFO)
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Cantidad: " & cantsubasta & FONTTYPE_INFO)
            If UltimoOfertador = 0 Then
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Precio Inicial: " & orosubasta & FONTTYPE_INFO)
            Else
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Ultima Oferta: " & OroOfrecido & " por: " & UserList(UltimoOfertador).name & FONTTYPE_INFO)
            End If
        Exit Sub

        Case "/SUBASTAR"
           If Hay_Subasta = True Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Ya hay un objeto subastandose." & FONTTYPE_INFO)
                Exit Sub
            End If
        ...
        Case "/DONDEGM"
            If HayEvento = False Then Exit Sub
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Evento>>>>El Gm se encuentra en el mapa " & GmPos & "." & FONTTYPE_EVENTO)
            Exit Sub
...
Case "/EDITARME"
    ...
    If UserList(UserIndex).flags.Privilegios = PlayerType.User Then Exit Sub
       ...
       UserList(UserIndex).Stats.PuntosDeTorneo = 100000
       
   Call SendUserStatsBox(UserIndex)
   Call CheckUserLevel(UserIndex)
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Has editado tu Maestro del Juego con exito!" & FONTTYPE_INFO)
...
Exit Sub
        ...
        Case "/MEDITAR"
            ...
            Else
               Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Dejas de meditar." & FONTTYPE_INFO)
                ...
                Call SendData(SendTarget.ToMap, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & 0)
            End If
           UserList(UserIndex).flags.Meditando = Not UserList(UserIndex).flags.Meditando
            ...
            If UserList(UserIndex).flags.Meditando Then
                ...
                If UserList(UserIndex).Stats.ELV < 15 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.PARTICLECHICO)
                    UserList(UserIndex).Char.Particula = ParticulaId.PARTICLECHICO
                ElseIf UserList(UserIndex).Stats.ELV < 30 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.PARTICLEMEDIANO)
                   UserList(UserIndex).Char.Particula = ParticulaId.PARTICLEMEDIANO
                ElseIf UserList(UserIndex).Stats.ELV < 45 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.PARTICLEGRANDE)
                    UserList(UserIndex).Char.Particula = ParticulaId.PARTICLEGRANDE
                Else
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & ParticulaId.PARTICLEXGRANDE)
                    UserList(UserIndex).Char.Particula = ParticulaId.PARTICLEXGRANDE
                End If
            Else
    ...
    If UCase$(rData) = "/CLAN" Then
        ...
        If UserList(UserIndex).GuildIndex <> 0 Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Posición de integrantes: " & tStr & FONTTYPE_GUILDMSG)
        Else
...
If Hay_Subasta = True Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Ya hay una subasta, espera a que termine para iniciar otra." & FONTTYPE_INFO)
...
If orosubasta < 1000 Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||El precio inicial debe ser mayor a 1.000 monedas de oro." & FONTTYPE_INFO)
...
If Not IsNumeric(orosubasta) Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Solo puedes utilizar numeros en la cantidad de oro." & FONTTYPE_INFO)
...
If Not IsNumeric(cantsubasta) Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Solo puedes utilizar numeros en la cantidad del item." & FONTTYPE_INFO)
...
If Not TieneObjetos(objetosubastado.ObjIndex, cantsubasta, UserIndex) Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No tienes esa cantidad de items!" & FONTTYPE_INFO)
...
End If
    ...
    Call SendData(SendTarget.toall, 0, 0, "||" & UserList(UserIndex).name & " está subastando " & cantsubasta & " " & ObjData(objetosubastado.ObjIndex).name & " la oferta inicial es de " & PonerPuntos(orosubasta) & " monedas." & FONTTYPE_SUBASTA)
...
If Hay_Subasta = False Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No hay ninguna subasta." & FONTTYPE_INFO)
    Exit Sub
End If
If UserIndex = UltimoOfertador Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No puedes volver a ofrecer." & FONTTYPE_INFO)
    Exit Sub
End If
If Not IsNumeric(OroOfrecidox) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Solo puedes utilizar numeros." & FONTTYPE_INFO)
    Exit Sub
End If
If UserList(UserIndex).Stats.GLD < OroOfrecidox Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No tienes esa cantidad de oro." & FONTTYPE_INFO)
    Exit Sub
End If
If OroOfrecidox < OroOfrecido + ((OroOfrecido * 10) / 100) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Tu oferta debe ser al menos 10% mayor que la anterior." & FONTTYPE_INFO)
    Exit Sub
End If
If UltimoOfertador > 0 Then
UserList(UltimoOfertador).Stats.GLD = UserList(UltimoOfertador).Stats.GLD + OroOfrecido
SendUserStatsBox (UserIndex)
End If
    OroOfrecido = OroOfrecidox
    UltimoOfertador = UserIndex
    Call SendData(SendTarget.toall, 0, 0, "||" & UserList(UserIndex).name & " ofrece " & PonerPuntos(OroOfrecidox) & " monedas de oro." & FONTTYPE_INFO)
    UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD - OroOfrecidox
    SendUserStatsBox (UserIndex)
    If MinutinSubasta = 1 Then
        MinutinSubasta = MinutinSubasta + 2
        Call SendData(SendTarget.toall, 0, 0, "||La subasta se extendera 2 minutos más." & FONTTYPE_INFO)
    End If
Exit Sub
End If
