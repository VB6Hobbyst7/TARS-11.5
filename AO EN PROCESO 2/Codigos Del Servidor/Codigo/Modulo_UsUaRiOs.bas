...
If HayEvento = True Then
If UserList(VictimIndex).flags.OrganizadorEvento = 1 Then
Call SendData(SendTarget.ToAll, 0, 0, "||Evento>>> " & UserList(AttackerIndex).name & " Ha derrotado a " & UserList(VictimIndex).name & " y es el ganador del evento." & FONTTYPE_EVENTO)
UserList(AttackerIndex).Stats.PuntosDeTorneo = UserList(AttackerIndex).Stats.PuntosDeTorneo + Evento
UserList(VictimIndex).flags.OrganizadorEvento = 0
UserList(VictimIndex).Stats.MinHP = UserList(VictimIndex).flags.ViejaVidaMin
UserList(VictimIndex).Stats.MaxHP = UserList(VictimIndex).flags.ViejaVidaMax
UserList(VictimIndex).Stats.MinMAN = UserList(VictimIndex).flags.ViejaManaMin
UserList(VictimIndex).Stats.MaxMAN = UserList(VictimIndex).flags.ViejaManaMax
HayEvento = False
Call SendUserStatsBox(VictimIndex)
Call SendData(SendTarget.ToIndex, VictimIndex, 0, "||Tu vida y mana ha vuelto a la anterior." & FONTTYPE_INFO)
End If
End If
...
Call SendData(SendTarget.ToIndex, AttackerIndex, 0, "FG" & UserList(AttackerIndex).Stats.UsuariosMatados)
...
Do While UserList(UserIndex).Stats.Exp >= UserList(UserIndex).Stats.ELU
    ...
    Dim conso As String
    conso = UserList(UserIndex).Char.CharIndex
    Call SendData(SendTarget.ToIndex, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbYellow & "°" & "Vida +" & AumentoHP & " Mana +" & AumentoMANA & " Golpe +" & AumentoHIT & "." & "°" & conso)
...
On Error GoTo ErrorHandler
    ...
    '<<Avisar a compañeros>>
    If UserList(UserIndex).GuildIndex > 0 Then
        Call SendData(SendTarget.ToGuildMembers, UserList(UserIndex).GuildIndex, 0, "||" & UserList(UserIndex).name & "ha muerto en el mapa" & UserList(UserIndex).Pos.Map & FONTTYPE_INFO)
    End If
...
Dim OldY As Integer
    ...
    If OldMap <> Map Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "TN" & MapInfo(Map).name)
