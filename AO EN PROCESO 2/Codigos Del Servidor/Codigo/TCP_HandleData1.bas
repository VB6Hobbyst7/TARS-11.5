...
Procesado = True 'ver al final del sub
    ...
    Select Case UCase$(Left$(rData, 1))
        ...
        Call SendData(SendTarget.ToAll, 0, 0, "||[Global] " & UserList(UserIndex).name & "> " & rData & FONTTYPE_GLOBAL)
        

Case "\"

            If UserList(UserIndex).flags.Muerto = 1 Then
                Call SendData(SendTarget.toindex, UserIndex, 0, "||¡¡Estas muerto!! Los muertos no pueden comunicarse con el mundo de los vivos. " & FONTTYPE_ROJO)
                Exit Sub
            End If
            ...
            If (EsDios(tName) Or EsAdmin(tName)) And UserList(UserIndex).flags.Privilegios < PlayerType.Dios Then
                Call SendData(SendTarget.toindex, UserIndex, 0, "||No puedes susurrarle a los Dioses y Admins." & FONTTYPE_ROJO)
                Exit Sub
            End If
            ...
            If UserList(UserIndex).flags.Privilegios = PlayerType.User And (EsSemiDios(tName) Or EsConsejero(tName)) Then
                Call SendData(SendTarget.toindex, UserIndex, 0, "||No puedes susurrarle a los GMs" & FONTTYPE_ROJO)
                Exit Sub
            End If
            ...
            If tIndex <> 0 Then
                ...
                End If
                    ...
                    Call SendData(SendTarget.toindex, UserIndex, UserList(UserIndex).Pos.Map, "||" & "Le has dicho a " & UserList(tIndex).name & "> " & tMessage & FONTTYPE_ROJO)

                    Call SendData(SendTarget.toindex, tIndex, UserList(UserIndex).Pos.Map, "||" & UserList(UserIndex).name & " te ha dicho> " & tMessage & FONTTYPE_ROJO)

                Exit Sub

            End If

            Call SendData(SendTarget.toindex, UserIndex, 0, "||Usuario inexistente. " & FONTTYPE_ROJO)
    ...
    Select Case UCase$(Left$(rData, 3))
           Case "ADF"
     rData = Right$(rData, Len(rData) - 3)
     Dim NickaddC As String
     Dim NumaddC As Byte
     NickaddC = ReadField(1, rData, 44)
     NumaddC = ReadField(2, rData, 44)
    Dim nombresitoamigo As String
    nombresitoamigo = UserList(UserIndex).name
     nombresitoamigo = UCase(nombresitoamigo)
     NickaddC = UCase(NickaddC)
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A1") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A2") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A3") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A4") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A5") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A6") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A7") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A8") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A9") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A10") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A11") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A12") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A13") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A14") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A15") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A16") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A17") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A18") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A19") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A20") Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Este usuario ya está en tu lista de contactos." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If Not FileExist(CharPath & NickaddC & ".chr", vbNormal) Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||El usuario no existe." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     If NickaddC = nombresitoamigo Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||No te podes agregar a vos mismo." & FONTTYPE_ERRORX)
        Exit Sub
     End If
     Call WriteVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A" & NumaddC, NickaddC)
    tStr = SendFriendList(UserIndex)
    Call SendData(SendTarget.toindex, UserIndex, 0, "ALS" & SendFriendList(UserIndex))
    Exit Sub
    Case "ELF"
     rData = Right$(rData, Len(rData) - 3)
     Dim NumelimC As String
     NumelimC = ReadField(1, rData, 44)
     Call WriteVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "AMIGOS", "A" & NumelimC, "(NADIE)")
    Exit Sub
        Case "USA"
            rData = Right$(rData, Len(rData) - 3)
...
'           rdata = Right$(rdata, Len(rdata) - 5)
    ...
    Case "OFDIOZ"
     rData = Right$(rData, Len(rData) - 6)
      ...
      'Acá si llega a x almas le da los items.
      If UserList(UserIndex).flags.AlmasOfrecidas + CantAlmas >= GetVar(App.Path & "\Dioses\Configuracion.ini", "INIT", "AlmasNecesarias") * UserList(UserIndex).flags.JerarquiaDios Then
      
            itemdios.Amount = 1
                
            If UCase$(UserList(UserIndex).Raza) = "GNOMO" Or UCase$(UserList(UserIndex).Raza) = "ENANO" Then
                itemdios.ObjIndex = GetVar(App.Path & "\Dioses\" & UserList(UserIndex).flags.SirvienteDeDios & "\Bajos.dat", "" & UserList(UserIndex).Clase & "", "Obj" & UserList(UserIndex).flags.JerarquiaDios)
            Else
                itemdios.ObjIndex = GetVar(App.Path & "\Dioses\" & UserList(UserIndex).flags.SirvienteDeDios & "\Altos.dat", "" & UserList(UserIndex).Clase & "", "Obj" & UserList(UserIndex).flags.JerarquiaDios)
            End If
      
         If Not MeterItemEnInventario(UserIndex, itemdios) Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Tenes el inventario lleno." & FONTTYPE_DIOSES)
            Exit Sub
         End If
         
         If UserList(UserIndex).flags.JerarquiaDios = 1 Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Tu jerarquia del dios " & UserList(UserIndex).flags.SirvienteDeDios & " cambió de Sirviente a Soldado." & FONTTYPE_GUILD)
         ElseIf UserList(UserIndex).flags.JerarquiaDios = 2 Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Tu jerarquia del dios " & UserList(UserIndex).flags.SirvienteDeDios & " cambió de Soldado a Guerrero." & FONTTYPE_GUILD)
        ElseIf UserList(UserIndex).flags.JerarquiaDios = 3 Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Tu jerarquia del dios " & UserList(UserIndex).flags.SirvienteDeDios & " cambió de Guerrero a Caballero." & FONTTYPE_GUILD)
        ElseIf UserList(UserIndex).flags.JerarquiaDios = 4 Then
            Call SendData(SendTarget.toindex, UserIndex, 0, "||Tu jerarquia del dios " & UserList(UserIndex).flags.SirvienteDeDios & " cambió de Caballero a Campeón." & FONTTYPE_GUILD)
        End If
         
         Call SendData(SendTarget.toindex, UserIndex, 0, "||Has conseguido " & ObjData(itemdios.ObjIndex).name & "." & FONTTYPE_ORO)
         UserList(UserIndex).flags.JerarquiaDios = UserList(UserIndex).flags.JerarquiaDios + 1
      ...
      Call SendData(SendTarget.toindex, UserIndex, 0, "||Sacrificaste " & CantAlmas & " almas al Dios " & UserList(UserIndex).flags.SirvienteDeDios & "." & FONTTYPE_DIOSES)
      ...
      ElseIf UserList(UserIndex).flags.SirvienteDeDios = "Terraske" Then
