...
Case eOBJType.otRuna
            ...
            If MapInfo(UserList(UserIndex).Pos.Map).Pk = False Then
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Este objeto solo puede ser utilizado en ciudades." & FONTTYPE_INFO)
                Exit Sub
            End If
            
            If UCase$(UserList(UserIndex).Hogar) = "ULLATHORPE" Then
                Call WarpUserChar(UserIndex, 1, 58, 46, True)
            ElseIf UCase$(UserList(UserIndex).Hogar) = "NIX" Then
                Call WarpUserChar(UserIndex, 34, 44, 88, True)
    ...
    Case eOBJType.otCristales
        If TieneObjetos(1274, 1, UserIndex) = False Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Necesitas un contenedor de almas para usar este objeto." & FONTTYPE_INFO)
         Exit Sub
        End If
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Has obtenido " & CristalitosNew & " almas oscuras." & FONTTYPE_ORO)
    ...
    Case eOBJType.otContenedor
        If UserList(UserIndex).flags.AlmasContenidas = 0 Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No tenes almas contenidas." & FONTTYPE_WARNING)
        Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Almas contenidas: " & UserList(UserIndex).flags.AlmasContenidas & "." & FONTTYPE_DIOSESN)
        End If
