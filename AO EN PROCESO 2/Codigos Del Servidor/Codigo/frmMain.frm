...
Call SendData(SendTarget.ToAll, 0, 0, "||La subasta finalizará en 3 minutos. Escribí /OFRECER (cantidad) para ofrecer o /INFORMARSUBASTA para ver los datos del objeto subastandose." & FONTTYPE_TSUBASTA)
ElseIf MinutinSubasta = 2 Then
Call SendData(SendTarget.ToAll, 0, 0, "||La subasta finalizará en 2 minutos. Escribí /OFRECER (cantidad) para ofrecer o /INFORMARSUBASTA para ver los datos del objeto subastandose." & FONTTYPE_TSUBASTA)
ElseIf MinutinSubasta = 1 Then
Call SendData(SendTarget.ToAll, 0, 0, "||La subasta finalizará en 1 minutos. Escribí /OFRECER (cantidad) para ofrecer o /INFORMARSUBASTA para ver los datos del objeto subastandose." & FONTTYPE_TSUBASTA)
ElseIf MinutinSubasta = 0 Then
Call SendData(SendTarget.ToAll, 0, 0, "||La subasta finalizará en 0 minutos. Escribí /OFRECER (cantidad) para ofrecer o /INFORMARSUBASTA para ver los datos del objeto subastandose." & FONTTYPE_TSUBASTA)
Timersubasta.Enabled = False
Hay_Subasta = False
If UltimoOfertador = 0 Then
Call MeterItemEnInventario(Subastador, objetosubastado)
Call SendData(SendTarget.ToAll, 0, 0, "||La subasta ha finalizado, el item no tuvo ninguna oferta." & FONTTYPE_TDSUBASTA)
Else
Call MeterItemEnInventario(UltimoOfertador, objetosubastado)
UserList(Subastador).Stats.GLD = UserList(Subastador).Stats.GLD + OroOfrecido
SendUserStatsBox (Subastador)
Call SendData(SendTarget.ToAll, 0, 0, "||La subasta finalizo, el item fue vendido a " & UserList(UltimoOfertador).name & " por " & PonerPuntos(OroOfrecido) & " monedas de oro." & FONTTYPE_TDSUBASTA)
End If
End If
End Sub
