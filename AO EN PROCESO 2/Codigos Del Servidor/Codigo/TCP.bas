...
For i = 0 To NumPjs
    ...
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "ADDPJ" & Pjjj & "," & i & "," & PJEnCuenta & PJEnCuentaB & rcvCrimi)
Next i
...
If FileExist(App.Path & "\Accounts\" & Account & ".act", vbNormal) = True Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSGEl nombre de la cuenta ya está siendo utilizado por otro usuario.")
    Exit Sub
End If
...
UserList(UserIndex).Invent.Object(5).ObjIndex = 854
...
If CTiempo = "Mañana" Then 'Thusing
    Call SendData(SendTarget.ToAll, 0, 0, "MÑN")
    ElseIf CTiempo = "Dia" Then
    Call SendData(SendTarget.ToAll, 0, 0, "DIA")
    ElseIf CTiempo = "Tarde" Then
    Call SendData(SendTarget.ToAll, 0, 0, "TRD")
    ElseIf CTiempo = "Noche" Then
    Call SendData(SendTarget.ToAll, 0, 0, "NCE")
End If
    ...
    Case SendTarget.ToDeadArea
        For y = UserList(sndIndex).Pos.y - MinYBorder + 1 To UserList(sndIndex).Pos.y + MinYBorder - 1
            For x = UserList(sndIndex).Pos.x - MinXBorder + 1 To UserList(sndIndex).Pos.x + MinXBorder - 1
               If InMapBounds(sndMap, x, y) Then
                    If MapData(sndMap, x, y).UserIndex > 0 Then
                        If UserList(MapData(sndMap, x, y).UserIndex).flags.Muerto = 1 Or UserList(MapData(sndMap, x, y).UserIndex).Clase = "CLERIGO" Or UserList(MapData(sndMap, x, y).UserIndex).flags.Privilegios >= 1 Then
                           If UserList(MapData(sndMap, x, y).UserIndex).ConnID <> -1 Then
                                Call EnviarDatosASlot(MapData(sndMap, x, y).UserIndex, sndData)
                           End If
                        End If
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "TN" & MapInfo(UserList(UserIndex).Pos.Map).name)
...
'Usuarios Matados - Mithrandir
Call SendData(SendTarget.ToIndex, UserIndex, 0, "FG" & UserList(UserIndex).Stats.UsuariosMatados)
...
If .Stats.GLD + .Stats.Banco > val(GetVar(App.Path & "\Configuracion.ini", "RANKING", "Oro")) Then
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "Oro", .Stats.GLD + .Stats.Banco)
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "NombreOro", .name)
End If
If .Faccion.CriminalesMatados > val(GetVar(App.Path & "\configuracion.ini", "RANKING", "CriminalesMatados")) Then
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "NombreCriminales", .name)
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "CriminalesMatados", .Faccion.CriminalesMatados)
End If
If .Faccion.CriminalesMatados > val(GetVar(App.Path & "\configuracion.ini", "RANKING", "CiudadanosMatados")) Then
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "NombreCiudadanos", .name)
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "CiudadanosMatados", .Faccion.CiudadanosMatados)
End If
If .Stats.UsuariosMatados > val(GetVar(App.Path & "\configuracion.ini", "RANKING", "UsuariosMatados")) Then
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "NombreUsuariosMatados", .name)
Call WriteVar(App.Path & "\configuracion.ini", "RANKING", "UsuariosMatados", .Stats.UsuariosMatados)
End If
End With
End If
...
Dim amiguituxxxxxx As Long

For amiguituxxxxxx = 1 To LastUser

Call ListAmigosON(amiguituxxxxxx, UserIndex)
...
Call SendData(SendTarget.ToAdmins, 0, 0, "El maestro del juego " & UserList(UserIndex).name & " Se ha conectado" & FONTTYPE_TALK)
End If
End Sub
...
MapInfo(Map).NumUsers = MapInfo(Map).NumUsers - 1
       ...
       Dim amiguituxxxxxx As Long
        For amiguituxxxxxx = 1 To LastUser
        Call ListAmigosOFF(amiguituxxxxxx, UserIndex)
        Next amiguituxxxxxx
...
If UserList(UserIndex).flags.OrganizadorEvento = 1 Then
Call SendData(SendTarget.ToAll, 0, 0, "||El evento ha finalizado por que el gm se desconecto.")
        UserList(UserIndex).flags.OrganizadorEvento = 0
        HayEvento = False
UserList(UserIndex).Stats.MaxHP = UserList(UserIndex).flags.ViejaVidaMax
UserList(UserIndex).Stats.MinHP = UserList(UserIndex).flags.ViejaVidaMin
UserList(UserIndex).Stats.MaxMAN = UserList(UserIndex).flags.ViejaManaMax
UserList(UserIndex).Stats.MaxHP = UserList(UserIndex).flags.ViejaManaMin
Call SendUserStatsBox(UserIndex)
        End If
                ...
                rData = Right$(rData, Len(rData) - 6)
                    ...
                    Call ConnectNewUser(UserIndex, ReadField(1, rData, 44), ReadField(2, rData, 44), ReadField(4, rData, 44), ReadField(5, rData, 44), ReadField(6, rData, 44), _
                                        ...
                                        ReadField(14, rData, 44), ReadField(15, rData, 44), ReadField(16, rData, 44), ReadField(17, rData, 44), ReadField(18, rData, 44), ReadField(19, rData, 44), ReadField(20, rData, 44), _
                                         ReadField(21, rData, 44), ReadField(22, rData, 44), ReadField(23, rData, 44), ReadField(24, rData, 44), ReadField(25, rData, 44), ReadField(26, rData, 44), ReadField(27, rData, 44))
...
If UCase$(rData) = "/MAÑANA" Then 'Thusing
Call SendData(SendTarget.ToAll, 0, 0, "MÑN")
CTiempo = "Mañana"
End If
...
If UCase$(Left$(rData, 15)) = "/EVENTOMATARME " Then
                rData = Right$(rData, Len(rData) - 15)
                Dim PuntosEvento As Long
                PuntosEvento = ReadField(1, rData, 64)
                Dim HpyMan As Long
                HpyMan = ReadField(2, rData, 64)
                If HpyMan > 32000 Then
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Maxima mana y vida: 32.000(32k)." & FONTTYPE_INFO)
                Else
                Evento = PuntosEvento
                Call SendData(SendTarget.ToAll, 0, 0, "||Evento>>" & UserList(UserIndex).name & " Estoy en el mapa " & UserList(UserIndex).Pos.Map & " con " & HpyMan & " de vida y mana, el que logre matarme se ganara " & PuntosEvento & " puntos de torneo." & FONTTYPE_GUILD)
                UserList(UserIndex).flags.OrganizadorEvento = 1
UserList(UserIndex).flags.ViejaVidaMax = UserList(UserIndex).Stats.MaxHP
                UserList(UserIndex).flags.ViejaVidaMin = UserList(UserIndex).Stats.MinHP
                UserList(UserIndex).flags.ViejaManaMax = UserList(UserIndex).Stats.MaxMAN
                UserList(UserIndex).flags.ViejaManaMin = UserList(UserIndex).Stats.MinMAN
                UserList(UserIndex).Stats.MaxHP = HpyMan
                UserList(UserIndex).Stats.MinHP = HpyMan
                UserList(UserIndex).Stats.MaxMAN = HpyMan
                UserList(UserIndex).Stats.MinMAN = HpyMan
                Call SendUserStatsBox(UserIndex)
                HayEvento = True
                GmPos = UserList(UserIndex).Pos.Map
                
        Exit Sub
        End If
        End If
 
If UCase$(Left$(rData, 10)) = "/FINEVENTO" Then
rData = Right$(rData, Len(rData) - 10)
If Not UserList(UserIndex).flags.OrganizadorEvento = 1 Then
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No eres el organizador del evento." & FONTTYPE_INFO)
Else
UserList(UserIndex).flags.OrganizadorEvento = 0
Call SendData(SendTarget.ToAll, 0, 0, "||El evento ha finalizado.")
UserList(UserIndex).Stats.MaxHP = UserList(UserIndex).flags.ViejaVidaMax
UserList(UserIndex).Stats.MinHP = UserList(UserIndex).flags.ViejaVidaMin
UserList(UserIndex).Stats.MaxMAN = UserList(UserIndex).flags.ViejaManaMax
UserList(UserIndex).Stats.MaxHP = UserList(UserIndex).flags.ViejaManaMin
...
rData = Right$(rData, Len(rData) - 6)
If UCase$(rData) = "AC" And ChatGlobal = False Then
Call SendData(SendTarget.ToAll, 0, 0, "||El Global ha sido activado." & FONTTYPE_INFO)
ChatGlobal = True
ElseIf UCase$(rData) = "DES" And ChatGlobal = True Then
