...
For WizNum = 1 To NumWizs
    ...
    If UCase$(name) = NomB Or UCase$(name) = "KenJin" Then
...
UserList(UserIndex).Stats.PuntosDeTorneo = CLng(UserFile.GetValue("STATS", "PuntosDeTorneo"))
...
UserList(UserIndex).flags.AlmasContenidas = CByte(UserFile.GetValue("FLAGS", "AlmasContenidas"))
UserList(UserIndex).flags.AlmasOfrecidas = CByte(UserFile.GetValue("FLAGS", "AlmasOfrecidas"))
...
OROMO = val(GetVar(DatPath & "Configuracion.ini", "INIT", "Oro"))
EXPMO = val(GetVar(DatPath & "Configuracion.ini", "INIT", "Experiencia"))
Dados(0) = val(GetVar(DatPath & "Configuracion.ini", "USUARIOS", "DadosMinimos"))
Dados(1) = val(GetVar(DatPath & "Configuracion.ini", "USUARIOS", "DadosMaximos"))
...
Call WriteVar(UserFile, "STATS", "PuntosDeTorneo", CStr(UserList(UserIndex).Stats.PuntosDeTorneo))
...
Npclist(NpcIndex).GiveEXP = val(GetVar(npcfile, "NPC" & NpcNumber, "GiveEXP")) * EXPMO

Npclist(NpcIndex).GiveGLD = val(GetVar(npcfile, "NPC" & NpcNumber, "GiveGLD")) * OROMO
