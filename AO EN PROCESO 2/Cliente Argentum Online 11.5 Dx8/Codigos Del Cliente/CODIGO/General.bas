...
Sub UsarMacro(f As Byte)
 
If Macros(f).comando <> "" Then
    Call SendData(CStr(Trim(Macros(f).comando)))
ElseIf Macros(f).Hechizo > 0 Then
    If frmMain.hlst.List(f - 10) <> "(None)" And UserCanAttack = 1 Then
        Call SendData("LH" & Macros(f).Hechizo)
        Call SendData("UK" & Magia)
        UsaMacro = True
    End If
ElseIf Macros(f).Equipar > 0 Then
    If (Macros(f).Equipar > 0) And (Macros(f).Equipar < MAX_INVENTORY_SLOTS + 1) Then _
        SendData "EQUI" & Macros(f).Equipar
ElseIf Macros(f).Usar > 0 Then
    If Not NoPuedeUsar Then
        NoPuedeUsar = True
        If (Macros(f).Usar > 0) And (Macros(f).Usar < MAX_INVENTORY_SLOTS + 1) Then _
            SendData "USA" & Macros(f).Usar
    End If
Else
    frmMacro.lblF.Caption = "Macro F" & f + 1
    frmMacro.numF.Caption = f + 1
    If Macros(f).comando <> "" Then
        frmMacro.optComando.value = True
        frmMacro.txtComando.Text = Macros(f).comando
    ElseIf Macros(f).Hechizo > 0 Then
...
Sub GuardarMacros()
 Dim A As Byte
For A = 0 To 9
    WriteVar App.path & "\Recursos\Init\macros.dat", "Macro" & A, "Comando", Macros(A).comando
    WriteVar App.path & "\Recursos\Init\macros.dat", "Macro" & A, "Usar", Macros(A).Usar
    WriteVar App.path & "\Recursos\\Init\macros.dat", "Macro" & A, "Equipar", Macros(A).Equipar
    WriteVar App.path & "\Recursos\Init\macros.dat", "Macro" & A, "Hechizo", Macros(A).Hechizo
Next A
 
End Sub
Sub CargarMacros(Optional Cargar As Boolean = True)
 
Dim A As Byte
 
For A = 0 To 9
    Macros(A).comando = GetVar(App.path & "\recursos\Init\macros.dat", "Macro" & A, "Comando")
    Macros(A).Usar = GetVar(App.path & "\recursos\Init\macros.dat", "Macro" & A, "Usar")
    Macros(A).Equipar = GetVar(App.path & "\recursos\Init\macros.dat", "Macro" & A, "Equipar")
    Macros(A).Hechizo = GetVar(App.path & "\recursos\Init\macros.dat", "Macro" & A, "Hechizo")
Next A
 For A = 0 To 9
    If Macros(A).comando <> "" Then
        'reemplazar por grafico de Comandos
        frmMain.picMacro(A).Picture = LoadPicture(App.path & "\Recursos\Graficos\531.bmp")
    ElseIf Macros(A).Equipar > 0 Then
        If Inventario.grhindex(Macros(A).Equipar) <> 0 Then
            If GrhData(Inventario.grhindex(Macros(A).Equipar)).FileNum <> 0 Then _
            frmMain.picMacro(A).Picture = LoadPicture(App.path & "\Recursos\Graficos\" & GrhData(Inventario.grhindex(Macros(A).Equipar)).FileNum & ".bmp")
        End If
    ElseIf Macros(A).Usar > 0 Then
        If Inventario.grhindex(Macros(A).Usar) <> 0 Then
            If GrhData(Inventario.grhindex(Macros(A).Usar)).FileNum <> 0 Then _
            frmMain.picMacro(A).Picture = LoadPicture(App.path & "\Recursos\Graficos\" & GrhData(Inventario.grhindex(Macros(A).Usar)).FileNum & ".bmp")
        End If
    ElseIf Macros(A).Hechizo > 0 Then
        'reemplazar por grafico de Hechizos
        frmMain.picMacro(A).Picture = LoadPicture(App.path & "\Recursos\Graficos\617.bmp")
    End If
Next A
End Sub
Function ColorToDX8(ByVal long_color As Long) As Long
