...
Select Case left(sData, 1)
    ...
    Select Case left$(sData, 2)
        Case "AS"
            ...
            frmMain.GldLbl.Caption = UserGLD & CalculateK(UserGLD)
        ...
        Case "CM"              ' >>>>> Cargar Mapa :: CM
            ...
            If FileExist(App.path & "\Recursos\MAPAS\" & "Mapa" & UserMap & ".map", vbNormal) Then
                Open App.path & "\Recursos\MAPAS\" & "Mapa" & UserMap & ".map" For Binary As #1
                Seek #1, 1
                Get #1, , tempint
                Close #1
'                If tempint = Val(ReadField(2, Rdata, 44)) Then
    ...
    End With
        ...
        Case "TW"          ' >>>>> Play un WAV :: TW
            ...
            Rdata = Right$(Rdata, Len(Rdata) - 2)
            NombreMapa = Rdata
            frmMain.NombreMapa.Caption = Rdata
            PuedeMapa = True
            Contador = 0
            Exit Sub
    ...
    Case "MÑN" 'Thusing
base_light = D3DColorARGB(255, 215, 215, 215)
Tiempo = "Mañana"
Exit Sub
 
Case "DIA"
base_light = D3DColorARGB(255, 255, 255, 255)
Tiempo = "Día"
Exit Sub
        
Case "TRD"
base_light = D3DColorARGB(255, 190, 190, 190)
Tiempo = "Tarde"
Exit Sub
        
Case "NCE"
base_light = D3DColorARGB(255, 125, 125, 125)
Tiempo = "Noche"
Exit Sub
...
#End If
        ...
        Case "CFF"                  ' Particulas para meditar by juanjo
            Rdata = Right$(Rdata, Len(Rdata) - 3)
            CharIndex = Val(ReadField$(1, Rdata, 44))
       ...
       Case "CFX"                  ' >>>>> Mostrar FX sobre Personaje :: CFX
        ...
        Case "EST"                  ' >>>>> Actualiza Estadisticas de Usuario :: EST
            ...
            frmMain.exp.Caption = "Exp:" & UserExp & "/" & UserPasarNivel
            frmMain.lblPorcLvl.Caption = "[" & Round(CDbl(UserExp) * CDbl(100) / CDbl(UserPasarNivel), 2) & "%]"
            frmMain.Hpshp.Width = (((UserMinHP / 100) / (UserMaxHP / 100)) * 86)
            frmMain.HpBar.Caption = UserMinHP & "/" & UserMaxHP
            If UserMaxMAN > 0 Then
                frmMain.MANShp.Width = (((UserMinMAN + 1 / 100) / (UserMaxMAN + 1 / 100)) * 86)
                frmMain.ManaBar.Caption = UserMinMAN & "/" & UserMaxMAN
            Else
                frmMain.MANShp.Width = 0
            End If
            
            frmMain.STAShp.Width = (((UserMinSTA / 100) / (UserMaxSTA / 100)) * 86)
            frmMain.StaBar.Caption = UserMinSTA & "/" & UserMaxSTA
            frmMain.GldLbl.Caption = UserGLD & CalculateK(UserGLD)
            frmMain.LvlLbl.Caption = UserLvl
...
#End If
    ...
    Case "GODS"
        ...
        frmGods.Picture = LoadPicture(App.path & "\Recursos\GRAFICOS\Principal\Almas_Main.jpg")
        
        If UCase$(SirvienteDe) = "MIFRIT" Then
         frmGods.imgGods.Picture = LoadPicture(App.path & "\Recursos\GRAFICOS\Principal\Almas_Dios2.jpg")
        ElseIf UCase$(SirvienteDe) = "TERRASKE" Then
         frmGods.imgGods.Picture = LoadPicture(App.path & "\Recursos\GRAFICOS\Principal\Almas_Dios4.jpg")
        ElseIf UCase$(SirvienteDe) = "EREBROS" Then
         frmGods.imgGods.Picture = LoadPicture(App.path & "\Recursos\GRAFICOS\Principal\Almas_Dios1.jpg")
        ElseIf UCase$(SirvienteDe) = "POSEIDON" Then
         frmGods.imgGods.Picture = LoadPicture(App.path & "\Recursos\GRAFICOS\Principal\Almas_Dios3.jpg")
        End If
        ...
        Case "EHYS"              ' Actualiza Hambre y Sed :: EHYS
            ...
            frmMain.AGUAsp.Width = (((UserMinAGU / 100) / (UserMaxAGU / 100)) * 86)
            frmMain.COMIDAsp.Width = (((UserMinHAM / 100) / (UserMaxHAM / 100)) * 86)
...
Sub Login(ByVal valcode As Integer)
    If EstadoLogin = Normal Then
        SendData ("OOLOGI" & UserName & "," & App.Major & "." & App.Minor & "." & App.Revision)
